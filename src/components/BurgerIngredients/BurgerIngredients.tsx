...
const BurgerIngredients: FC = () => {
  ...
  const bun = React.useRef<null | HTMLElement>(null);
  const sauce = React.useRef<null | HTMLElement>(null);
  const main = React.useRef<null | HTMLElement>(null);

  const bunScroll = (id: string) => {
    setCurrent(id);
    bun!.current!.scrollIntoView({ behavior: "smooth" })
  }
  const sauceScroll = (id: string) => {
    setCurrent(id);
    sauce!.current!.scrollIntoView({ behavior: "smooth" })
  } 
  const mainScroll = (id: string) => {
    setCurrent(id);
